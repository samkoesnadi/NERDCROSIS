...
from app.necro_segment import necro_segment
...
def d_necro_seg(oris, labels):
    ...
    for i, label_ready in enumerate(labels_ready):
        ...
        index = 0
        for i, (ori, label) in enumerate(zip(ori_ROI,label_ROI)):
            #if (i!=4) : continue
            try:
            ...
            #end_label[end_label>0] *= i+1+a
            end_label[end_label==2] = 3+i
            end_label[end_label==1] = 1+i
...
def main_necro_segment(mypath, oris_path, out_path):
    '''
    mypath is the path for the femur segmentation
    oris_path is the path of the whole 3d MRI
    out_path is the output path
    '''
    # mypath = 'models/1/output'
    # oris_path = "data/"
    ...
    for segment_filename in segment_filenames:
        ...
        print("Writing neucro segment in ",join(out_path, 'P'+prefix+"necro_seg.gipl"))
        itk.imwrite(new_segment, join(out_path, 'P'+prefix+"necro_seg.gipl"))
        yield (segment, 'P'+segment_filename[0:-20]+'_necro_seg.gipl')

def main_necro_segment_fiveclasses(mypath, oris_path, out_path):
    '''
    mypath is the path for the femur segmentation
    oris_path is the path of the whole 3d MRI
    out_path is the output path
    ...
    for segment_filename in segment_filenames:
        ...
        #plt.imshow(data[10]), plt.show()
        #print(data.shape)
        # segment = segment.astype(np.float32)
        segment = data.astype(np.uint8)
