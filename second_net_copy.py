...
ready_model = get_resnet()

print(ready_model.summary())
#from keras import optimizers

#adam = optimizers.Adam(lr=0.003, amsgrad=True)
#ready_model.compile(optimizer=adam, loss=dice_coef_loss, metrics=[dice_coef])
...
d = np.append(d, np.load('npy_middle/9_0.npy'), axis=0)
d = np.append(d, np.load('npy_middle/10_0.npy'), axis=0)
d = np.append(d, np.load('npy_middle/11_0.npy'), axis=0)
d = np.append(d, np.load('npy_middle/12_1.npy'), axis=0)
d = np.append(d, np.load('npy_middle/13_1.npy'), axis=0)
d = np.append(d, np.load('npy_middle/14_1.npy'), axis=0)
d = np.append(d, np.load('npy_middle/15_1.npy'), axis=0)
d = np.append(d, np.load('npy_middle/16_1.npy'), axis=0)
d = np.append(d, np.load('npy_middle/13_0.npy'), axis=0)
d = np.append(d, np.load('npy_middle/14_0.npy'), axis=0)
d = np.append(d, np.load('npy_middle/15_0.npy'), axis=0)
d = np.append(d, np.load('npy_middle/16_0.npy'), axis=0)
d = np.append(d, np.load('npy_middle/17_0.npy'), axis=0)
...
imgs_bit_mask_train = y_train.astype('float32')
...
x_train = np.reshape(x_train, (len(x_train), img_rows, img_cols, 1))  # adapt this if using `channels_first` image data format
y_train = np.reshape(y_train, (len(y_train), img_rows, img_cols, 1))
x_test = np.reshape(x_test, (len(x_test), img_rows, img_cols, 1))  # adapt this if using `channels_first` image data format
y_test = np.reshape(y_test, (len(y_test), img_rows, img_cols, 1))
...
ready_model.save_weights("model_second_net.h5")
...
decoded_imgs = ready_model.predict(x_test,verbose=1)
...
for i in range(n):
    ...
    plt.imshow(x_test[i].reshape(img_rows,img_cols)), plt.show()
    plt.imshow(decoded_imgs[i].reshape(img_rows,img_cols)), plt.show()
