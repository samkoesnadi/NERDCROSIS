...
def second_network(x):
    ...
    #conv3 = layers.
    decoded = layers.concatenate([decoded, layers.ZeroPadding2D(((12,13),(12,13)))(conv3)])
    decoded = layer_finishing(decoded)
    decoded = layers.Conv2DTranspose(8, kernel_size=(3, 3), strides=1, dilation_rate=2)(decoded)
    decoded = layers.concatenate([decoded, layers.ZeroPadding2D(((11,12),(11,12)))(conv2)])
    decoded = layers.BatchNormalization()(decoded)

    decoded = layers.Conv2DTranspose(4, kernel_size=(4, 4), strides=1, dilation_rate=2)(decoded)
    decoded = layer_finishing(decoded)
    decoded = layers.Dropout(0.5)(decoded)
    decoded = layers.Conv2DTranspose(4, kernel_size=(3, 3), strides=1, dilation_rate=2)(decoded)
    decoded = layer_finishing(decoded)
    decoded = layers.Dropout(0.5)(decoded)
...
ready_model.save_weights("model_second_net3.h5")
...
ready_model.load_weights("model_second_net3.h5")
