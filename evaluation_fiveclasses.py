...
config.set('SEGMENTATION', 'num_classes', str(5))
...
from app.threed_necro_segment import main_necro_segment_fiveclasses
f = open(join(prediction_dir,"log_accuracy.txt"),"w+")
f.seek(0)

f.write("Evaluation:\n\n")
for prediction, name in main_necro_segment_fiveclasses(join(model_dir,save_seg_dir), ori_dir, join(prediction_dir,'necro_seg')):
    ...
    # just see necrosis
    np.where((prediction==3)|(prediction==4),1,0)
