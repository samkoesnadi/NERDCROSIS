...
def second_network(x):
    ...
    def grouped_convolution(y, nb_channels, _strides):
        return layers.Conv2D(nb_channels, kernel_size=(3, 3), strides=_strides, padding='same',
                kernel_regularizer=regularizers.l2(0.01),
                activity_regularizer=regularizers.l1(0.01))(y)
    ...
    def resblock(y, nb_channels_in, nb_channels_out, _strides=(1, 1), _project_shortcut=False):
        ...
        if _project_shortcut or _strides != (1, 1):
            ...
            collectr = layers.Conv2D(nb_channels_out, kernel_size=(1, 1), strides=_strides, padding='same')(collectr)
            collectr = layers.BatchNormalization()(collectr)
    ...
    # conv1
    x = layers.Conv2D(4, kernel_size=(5, 5), strides=(2, 2), padding='same')(x)
    x = layer_finishing(x)
    ...
    for i in range(1):
        ...
        x = resblock(x, 8, 16, _project_shortcut=project_shortcut)

    # conv3
    for i in range(2):
        # down-sampling is performed by conv3_1, conv4_1 with a stride of 2
        strides = (2, 2)
        x = resblock(x, 16, 32, _strides=strides)

    encoded = x

    decoded = layers.Conv2DTranspose(21, (15, 15), strides=4, use_bias=False)(encoded)
    decoded = layer_finishing(decoded)
    decoded = layers.Conv2D(16, kernel_size=(3, 3), strides=1, padding='same')(decoded)
    decoded = layer_finishing(decoded)
    decoded = layers.Conv2D(8, kernel_size=(3, 3), strides=1, padding='same')(decoded)
    decoded = layer_finishing(decoded)
    decoded = layers.Conv2DTranspose(8, (8, 8), strides=2, use_bias=False)(decoded)
    decoded = layer_finishing(decoded)
    ...
    # decoded = layer_finishing(decoded)
    # decoded = layers.UpSampling2D((2, 2))(decoded)
    decoded = layers.Conv2D(1, kernel_size=(1, 1), strides=1, padding='same')(decoded)
    decoded = layer_finishing(decoded)
